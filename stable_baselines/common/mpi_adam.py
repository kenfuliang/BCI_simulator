...
class MpiAdam(object):
    def __init__(self, var_list, *, beta1=0.9, beta2=0.999, epsilon=1e-08, scale_grad_by_procs=True, comm=None,
        ...
        self.exp_avg = np.zeros(size, 'float64')
        # Exponential moving average of squared gradient values
        # "second raw moment estimate" v in the paper
        self.exp_avg_sq = np.zeros(size, 'float64')
        self.step = 0
        self.setfromflat = tf_utils.SetFromFlat(var_list, dtype=tf.float64, sess=sess)
    ...
    def update(self, local_grad, learning_rate):
        ...
        if self.step % 100 == 0:
            self.check_synced()
        local_grad = local_grad.astype('float64')
...
def test_mpi_adam():
    ...
    a_var = tf.Variable(np.random.randn(3).astype('float64'))
    b_var = tf.Variable(np.random.randn(2, 5).astype('float64'))
