...
def observation_input(ob_space, batch_size=None, name='Ob', scale=False):
    ...
    if isinstance(ob_space, Discrete):
        ...
        processed_observations = tf.cast(tf.one_hot(observation_ph, ob_space.n), tf.float64)
        ...
        processed_observations = tf.cast(observation_ph, tf.float64)
        ...
        processed_observations = tf.concat([
            tf.cast(tf.one_hot(input_split, ob_space.nvec[i]), tf.float64) for i, input_split
