...
class DiagGaussianProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_latent(self, pi_latent_vector, vf_latent_vector, init_scale=1.0, init_bias=0.0):
        ...
        logstd = tf.get_variable(name='pi/logstd', shape=[1, self.size], initializer=tf.zeros_initializer(), dtype=np.float64)
...
class BernoulliProbabilityDistribution(ProbabilityDistribution):
    ...
    def sample(self):
        ...
        return tf.cast(math_ops.less(samples_from_uniform, self.probabilities), tf.float64)
