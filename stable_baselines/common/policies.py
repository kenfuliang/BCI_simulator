...
def mlp_extractor(flat_observations, net_arch, act_fun):
    ...
    of them are shared between the policy network and the value network. It is assumed to be a list with the following structure:
...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,
        ...
        with tf.variable_scope("input", reuse=False):
            self._dones_ph = tf.placeholder(tf.float64, (n_batch, ), name="dones_ph")  # (done t-1)
            state_ph_shape = (self.n_env, ) + tuple(state_shape)
            self._states_ph = tf.placeholder(tf.float64, state_ph_shape, name="states_ph")

        initial_state_shape = (self.n_env, ) + tuple(state_shape)
        self._initial_state = np.zeros(initial_state_shape, dtype=np.float64)
