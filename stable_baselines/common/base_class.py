...
class BaseRLModel(ABC):
    ...
    def pretrain(self, dataset, n_epochs=10, batch_size=64, learning_rate=1e-4,
        ...
        if self.verbose > 0:
            print("Pretraining with Behavior Cloning with batch_size {}...".format(batch_size))

        for epoch_idx in range(int(n_epochs)):
            train_loss = 0.0
            # Full pass on the training set
            for _ in range(len(dataset.train_set)//batch_size):
                expert_obs, expert_actions = dataset.get_next_batch(batch_size,split='train')
                feed_dict = {
                    obs_ph: expert_obs,
            ...
            train_loss /= len(dataset.train_set)//batch_size

            if self.verbose > 0 and (epoch_idx + 1) % val_interval == 0:
                val_loss = 0.0
                # Full pass on the validation set
                for _ in range(len(dataset.val_set)//batch_size):
                    expert_obs, expert_actions = dataset.get_next_batch(batch_size, split='val')
                    val_loss_, = self.sess.run([loss], {obs_ph: expert_obs,
                                                        actions_ph: expert_actions})
                    val_loss += val_loss_

                val_loss /= len(dataset.val_set)//batch_size
                if self.verbose > 0:
