...
def ortho_init(scale=1.0):
    ...
    def _ortho_init(shape, *_, **_kwargs):
        ...
        return (scale * weights[:shape[0], :shape[1]]).astype(np.float64)
...
def linear(input_tensor, scope, n_hidden, *, init_scale=1.0, init_bias=0.0):
    ...
    with tf.variable_scope(scope):
        ...
        weight = tf.get_variable("w", [n_input, n_hidden], initializer=ortho_init(init_scale), dtype=np.float64)
        bias = tf.get_variable("b", [n_hidden], initializer=tf.constant_initializer(init_bias), dtype=np.float64)
...
def lstm(input_tensor, mask_tensor, cell_state_hidden, scope, n_hidden, init_scale=1.0, layer_norm=False):
    ...
    with tf.variable_scope(scope):
        weight_x = tf.get_variable("wx", [n_input, n_hidden * 4], initializer=ortho_init(init_scale), dtype=tf.float64)
        weight_h = tf.get_variable("wh", [n_hidden, n_hidden * 4], initializer=ortho_init(init_scale), dtype=tf.float64)
        bias = tf.get_variable("b", [n_hidden * 4], initializer=tf.constant_initializer(0.0), dtype=tf.float64)
